...
public class Spec_Test
{
    ...
    {
        Assert.AreEqual(YamlDocumentTest, Spec.Serializer.Serialize(InMemoryTest));
    }

    [Test]
    public void DeserializerMatches()
    {
        Dictionary<TestEnum, TestObject> deserialized = Spec.Deserializer.Deserialize<Dictionary<TestEnum, TestObject>>(YamlDocumentTest);
